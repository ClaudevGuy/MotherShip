...
function FileTreeNode({ node, depth = 0 }: { node: TreeNode; depth?: number }) {
  ...
  return (
    <div>
      ...
      >
        {isFolder ? (
          open ? <FolderOpen className="size-3.5 text-[#00d992] shrink-0" /> : <FolderClosed className="size-3.5 text-[#00d992]/60 shrink-0" />
        ) : (
...
const COMMIT_TYPE_COLORS: Record<string, { bg: string; border: string; text: string }> = {
  ...
  refactor: { bg: "rgba(0,217,146,0.12)", border: "rgba(0,217,146,0.3)", text: "#00d992" },
};
        ...
        <div className="flex gap-6">
          {tabs.map((t) => (
            ...
            >
              {t.label}
              {tab === t.id && (
                <div className="absolute bottom-0 left-0 right-0 h-[2px] bg-[#00d992]" />
              )}
            </button>
          ))}
        </div>
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {repositories.map((repo) => (
              <GlassPanel key={repo.id} hover padding="lg" className="transition-shadow hover:shadow-[0_0_12px_rgba(0,217,146,0.08)]">
                <div className="space-y-3">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <SparklineChart data={[3,5,2,7,4,6,8]} color="#00d992" width={40} height={16} />
                  </div>
            ...
            <GlassPanel padding="lg" className="lg:col-span-3">
              ...
              <div className="mt-5 pt-4 border-t border-border">
                ...
                <div className="space-y-1.5">
                  ...
                  <p className="text-xs text-[#00d992]/80">↑ Technical debt trending down — 3 modules refactored</p>
                </div>
              </div>
            </GlassPanel>
          </div>
              ...
              {commits.slice(0, 8).map((c) => {
                ...
                return (
                  <div key={c.id} className="flex items-center gap-3 py-2 px-2 rounded-lg hover:bg-muted/30 transition-colors">
                    <span className="font-mono text-xs text-[#00d992] shrink-0 w-16">{c.hash}</span>
                    <span
                    ...
                    <div className="flex items-center gap-1 shrink-0">
                      {c.isAgent && <Bot className="size-3 text-[#00d992]" />}
