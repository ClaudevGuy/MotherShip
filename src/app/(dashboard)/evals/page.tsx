  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-start justify-between">
        ...
        <Button size="sm" className="bg-[#00d992] hover:bg-[#00d992]/90 text-black gap-1.5" onClick={() => setCreateOpen(true)}>
      ...
      {suites.length > 0 && (
        <div className="grid grid-cols-4 gap-3">
          <div className="rounded-xl border border-border bg-card p-3 flex items-center gap-3">
            <div className="flex size-8 items-center justify-center rounded-lg bg-[#00d992]/10"><Hash className="size-3.5 text-[#00d992]" /></div>
              ...
              >
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity" onClick={(e) => e.stopPropagation()}>
                    <button onClick={() => handleRun(suite.id)} disabled={isRunning} className="flex items-center gap-1 rounded-md px-2 py-1 text-[10px] font-medium text-[#00d992] bg-[#00d992]/10 hover:bg-[#00d992]/20 transition-colors disabled:opacity-40">
      ...
      <div className="w-[600px] max-h-[85vh] rounded-xl border border-border bg-card shadow-2xl flex flex-col" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-border/50 shrink-0">
          ...
          <div className="flex gap-1">
            {[0, 1, 2].map(i => (
              <div key={i} className={cn("h-1 w-8 rounded-full", i <= wizardStep ? "bg-[#00d992]" : "bg-muted/30")} />
            ))}
          </div>
        </div>
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-1.5 mb-2">
                    {caseCriteria.map((c, i) => (
                      <span key={i} className="flex items-center gap-1 rounded-full bg-[#00d992]/10 px-2.5 py-1 text-[10px] font-medium text-[#00d992]">
                        {c}
                        <button onClick={() => setCaseCriteria(caseCriteria.filter((_, j) => j !== i))} className="text-[#00d992]/50 hover:text-[#00d992]">×</button>
                      </span>
                    ))}
                  </div>
                  {/* Suggestions */}
          ...
          {wizardStep === 2 && (
            <div className="space-y-4">
              ...
              <label className="flex items-center gap-2 cursor-pointer">
                <input type="checkbox" checked={runAfterCreate} onChange={(e) => setRunAfterCreate(e.target.checked)} className="rounded accent-[#00d992]" />
        ...
        <div className="flex items-center justify-between px-6 py-4 border-t border-border/50 shrink-0">
          ...
          {wizardStep < 2 ? (
            ...
            <Button size="sm" className="bg-[#00d992] hover:bg-[#00d992]/90 text-black" onClick={handleCreate} disabled={creating}>
