  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-start justify-between">
        ...
        <Button size="sm" className="bg-[#00d992] hover:bg-[#00d992]/90 text-black gap-1.5" onClick={handleRun} disabled={running}>
        ...
        <div className="flex gap-6">
          {(["overview", "cases", "history"] as const).map(t => (
            <button key={t} onClick={() => setTab(t)} className={cn("pb-2.5 text-sm font-medium capitalize relative transition-colors", tab === t ? "text-foreground" : "text-muted-foreground hover:text-foreground/70")}>
              ...
              {tab === t && <div className="absolute bottom-0 left-0 right-0 h-[2px] bg-[#00d992]" />}
            </button>
          ))}
        </div>
      </div>
      ...
      {tab === "cases" && (
        <div className="space-y-4">
          ...
          {addOpen && (
            <div className="rounded-lg border border-[#00d992]/20 bg-[#00d992]/[0.02] p-4 space-y-3">
              ...
              <div className="flex flex-wrap gap-1">
                {newCriteria.map((c, i) => (
                  <span key={i} className="flex items-center gap-1 rounded-full bg-[#00d992]/10 px-2 py-0.5 text-[10px] text-[#00d992]">
                    {c} <button onClick={() => setNewCriteria(newCriteria.filter((_, j) => j !== i))}>×</button>
                  </span>
                ))}
              </div>
              <div className="flex gap-2">
                <Button size="sm" className="bg-[#00d992] hover:bg-[#00d992]/90 text-black text-xs" onClick={addCase}>Add Case</Button>
